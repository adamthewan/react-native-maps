...
public class OsmMapView extends MapView implements MapView.OnFirstLayoutListener {
  ...
  public OsmMapView(ThemedReactContext reactContext, ReactApplicationContext appContext, OsmMapManager manager) {
    ...
    scaleDetector = new ScaleGestureDetector(reactContext, new ScaleGestureDetector.SimpleOnScaleGestureListener() {
      @Override
  ...
  /*
   * Keep a cache of map listeners because the original list is destroyed when
  ...
  public void setRegion(ReadableMap region) {
    ...
    BoundingBox bounds = new BoundingBox(lat + latDelta / 2, lng + lngDelta / 2, lat - latDelta / 2,
        lng - lngDelta / 2);
  ...
  public void addFeature(View child, int index) {
    ...
    if (child instanceof OsmMapMarker) {
       ...
       * else { ViewGroup children = (ViewGroup) child; for (int i = 0; i <
       * children.getChildCount(); i++) { addFeature(children.getChildAt(i), index); }
       * }
       */
  }
