...
public class OsmMapUrlTile extends OsmMapFeature {
  ...
  public class OsmMapTileSource extends OnlineTileSourceBase {
    ...
    public OsmMapTileSource(final String aName, final int aZoomMinLevel, final int aZoomMaxLevel,
        final int aTileSizePixels, final String urlTemplate) {
      super(aName, aZoomMinLevel, aZoomMaxLevel, aTileSizePixels, null, new String[] { urlTemplate }, null);
    }
    ...
    public String getTileURLString(final long pMapTileIndex) {
      String url = getBaseUrl().replace("{x}", Integer.toString(MapTileIndex.getX(pMapTileIndex)))
    ...
    public String getTileRelativeFilenameString(long pMapTileIndex) {
      return pathBase().replace("{x}", Integer.toString(MapTileIndex.getX(pMapTileIndex)))
          .replace("{y}", Integer.toString(MapTileIndex.getY(pMapTileIndex)))
          .replace("{z}", Integer.toString(MapTileIndex.getZoom(pMapTileIndex)));
    }
  }
  ...
  public void addToMap(MapView map) {
    final ITileSource tileSource = new OsmMapTileSource("OsmMapTileSource", (int) minimumZ, (int) maximumZ, 256,
        urlTemplate);
