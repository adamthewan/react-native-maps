...
public class OsmMapPolygon extends OsmMapFeature {
  ...
  public void setCoordinates(ReadableArray coordinates) {
    // it's kind of a bummer that we can't run map() or anything on the
    // ReadableArray
    this.coordinates = new ArrayList<>(coordinates.size() + 1);
    for (int i = 0; i < coordinates.size(); i++) {
      ReadableMap coordinate = coordinates.getMap(i);
      this.coordinates.add(i, new GeoPoint(coordinate.getDouble("latitude"), coordinate.getDouble("longitude")));
    }
